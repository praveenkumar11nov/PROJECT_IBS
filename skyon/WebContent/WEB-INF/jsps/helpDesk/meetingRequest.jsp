...
<!-----------------------urls   -------------------------------->
<c:url value="/meetingRequest/read" var="readUrl" />
<c:url value="/meetingRequest/add" var="createUrl" />
<c:url value="/meetingRequest/update" var="updateUrl" />
<c:url value="/meetingRequest/delete" var="destroyUrl" />
...
<kendo:grid name="notiGrid" pageable="true" edit="customerCareEvent" resizable="true" sortable="true" reorderable="true" selectable="true" scrollable="true">
		...
		<kendo:grid-filterable extra="false">
		 <kendo:grid-filterable-operators>
		  ...
		  <kendo:grid-editable mode="popup"
			confirmation="Are you sure you want to remove this Meeting Request?" />
		<kendo:grid-toolbar>
		
			<kendo:grid-toolbarItem name="create" text="Add Meeting Request" />
		</kendo:grid-toolbar>
		...
		<kendo:grid-column title="Block Names *" field="blockName" width="100px" editor="blockEditor" filterable="false">
		</kendo:grid-column>
		<kendo:grid-column title="Property Numbers *" field="propertyNo" width="80px" editor="proprtyEditor" filterable="false">
		</kendo:grid-column>
		<kendo:grid-column hidden="true"/>
		<kendo:grid-column title="Meeting Subject" field="meetingSubject" width="120px" editor="subjectEditor" filterable="false"/>
		<kendo:grid-column title="Meeting Message" field="MeetingMessage" width="120px" editor="messageEditor" filterable="false"/>
		<kendo:grid-column title="Meeting Location" field="location" width="120px" editor="locationEditor" filterable="false"/>
		<kendo:grid-column title="Meeting Start Time" field="startTime" editor="startDateEditor" filterable="false" width="90px" />
		<kendo:grid-column title="Meeting End Time" field="endTime"  editor="endDateEditor" filterable="false" width="90px" /> 
		<%-- <kendo:grid-column title="Status" field="meetingStatus" width="50px" /> --%>
		...
		<kendo:grid-column title=" " width="110px"> 
			...
			<%-- <kendo:dataSource-transport>
			 ...
			 <kendo:dataSource-schema>
				<kendo:dataSource-schema-model id="meeting_id">
					<kendo:dataSource-schema-model-fields>
					<kendo:dataSource-schema-model-field name="meeting_id" type="number" />
...
{
    $('<textarea name="location1" data-text-field="location" data-value-field="location" data-bind="value:' + options.field + '" style="width: 240px; height: 66px;" placeholder=" "/>')
         .appendTo(container);
   /*  $('<span class="k-invalid-msg" data-for="ssss"></span>').appendTo(container);  */
}
...
function edit(){
	...
	$(".k-window-title").text("Add Meeting Request");
...
function customerCareEvent(e)
{
	...
	if (e.model.isNew()){
		...
		$(".k-window-title").text("Add Meeting Request Details");
	     	 $(".k-grid-update").text("Save"); 
			 ...
			 $(".k-window-title").text("Edit Meeting Request Details");
		 ...
		 if (e.response.status == "CREATEFAIL") 
			{
			 ...
			 $("#alertsBox").html("Meeting Request created successfully");
				$("#alertsBox").dialog({
					modal : true,
					buttons : {
						"Close" : function() {
			 ...
			 $("#alertsBox").html("");
				$("#alertsBox").html("Meeting Request deleted successfully");
						$("#alertsBox").dialog({
							modal : true,
							buttons : {
								"Close" : function() {
			 ...
			 $("#alertsBox").html("");
				$("#alertsBox").html("Meeting Request updated successfully");
						$("#alertsBox").dialog({
							modal : true,
							buttons : {
								"Close" : function() {
