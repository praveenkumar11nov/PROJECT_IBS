...
<c:url value="/tariff/read" var="transportReadUrl" />
...
<div style="padding-left: 50px" class="fluid">
	...
	<div class="widget grid8" >
		...
		<kendo:treeView name="treeview" dataTextField="tariffName"
			select="onSelect" 
			template=" #: item.tariffName #  <br>#:item.status # <input type='hidden' id='syed' class='#: item.tariffName ##: item.elTariffID #' value='#: item.elTariffID#'/>">
			...
			<kendo:dataSource>
				...
				<kendo:dataSource-schema>
					<kendo:dataSource-schema-hierarchical-model id="elTariffID"
...
<div id="all" >
		<%-- <kendo:treeView name="treeview1" dataTextField="tariffName"
			select="onSelect" 
			template=" #: item.tariffName #<br>#:item.tariffCode #<br>#:item.tariffDescription #  <br>#:item.status # <input type='hidden' id='syed' class='#: item.tariffName ##: item.elTariffID#' value='#: item.elTariffID#'/>"
			>
				...
				<table style="height: 411px;">
					...
					<tr>					
						...
						<td><input  id="tariffCode" name="tariffCode" placeholder="Enter TariffCode" class="k-textbox"  
						  required="required"  validationMessage="Traiff Code Is Required"
						 /></td>
					</tr>					
					<tr>
					<td>From-Date*</td>
					<td> <kendo:datePicker format="yyyy/MM/dd  "  name="validFrom" id="validFrom"  parse="parse()" value="${today}"  
					change="startChange"
					 required="required" validationMessage="From Date Is Required">
					</kendo:datePicker><td>
					</td>
					</tr>
				<!-- 	<tr>
					...
					<td> <kendo:datePicker format="yyyy/MM/dd  " name="validTo" id="validTo"  parse="parse()" value="${today}"  
					 change="endChange"					 
					 required="required" validationMessage="TO-Date  Is Required">
					</kendo:datePicker></td>
					</tr>
					...
					<td><kendo:dropDownList name="tariffNodetype" id="tariffNodetype" dataTextField="name"
								...
								required="required"  optionLabel="Tariff Node Type" validationMessage="Traiff Node Type Is Required"  >
					...
					<td style="vertical-align:middle;">	<textarea  id="tariffDescription"
							name="tariffDescription" class="k-textbox" 
							 required="required" class="validate[required]" validationMessage="Traiff Description Is Required"
							></textarea></td>
					...
					<tr>
						...
						<td><kendo:dropDownList name="status" id="status" dataTextField="name"
								dataValueField="value" 
								required="required"  optionLabel="Status" validationMessage="Status Is Required">
						...
						<td></td>
						<td>
						<button type="submit"  id="appendNodeToSelected" class="buttonS bLightBlue">Save Tariff</button>
  ...
  <div id="updateperm" title="Update Tariff" align="center" style="display: none"> 
		<br/>	
				...
				<table style="height: 411px;">
					<tr>
						<td>Tariff Name*</td>
						<td><input id="editNodeText" class="k-textbox" required="required" validationMessage="Traiff NameIs Required"/></td>
					</tr>
					
					<tr>
						<td>Tariff Code*</td>
						<td><input  id="tariffcodeu" name="tariffcodeu " class="k-textbox" required="required" validationMessage="Traiff Code Is Required"/></td>
					</tr>
					...
					change="startChangeu" class="validate[required]" required="required" validationMessage="From-Date Is Required" style="aria-readonly=true;"></kendo:datePicker><td>
					</td>
					</tr>
					
					<tr>
					<td>To-Date*</td>
					<td> <kendo:datePicker format="yyyy/MM/dd " name="todateu" id="todateu" value="${today}" change="endChangeu"
					class="validate[required]" required="required" validationMessage="TO-Date Is Required" style="aria-readonly=true;"></kendo:datePicker></td>
					</tr>
					
					<tr>
					<td>Tariff Node Type*</td>
					<td><kendo:dropDownList name="tariffnodetypeu" id="tariffnodetypeu" dataTextField="name"
								dataValueField="value"  optionLabel="Tariff Node Type" required="required" validationMessage="Traiff Node Type Is Required">
								<kendo:dataSource data="${tariffNodetype}"></kendo:dataSource>
							</kendo:dropDownList></td>
					<tr>
					...
					<td style="vertical-align:middle;">	<textarea  id="tariffdescriptionu"
							name="tariffdescriptionu " class="k-textbox" 
							class="validate[required]" required="required" validationMessage="Traiff Description  Is Required"
							></textarea></td>
					...
					<tr>
						...
						<td><kendo:dropDownList name="statusu" id="statusu" dataTextField="name"
								dataValueField="value"  optionLabel="Status" required="required">
								<kendo:dataSource data="${status}"></kendo:dataSource>
							</kendo:dropDownList></td>
					</tr>
					<tr>				
						...
						<td>
							<!-- <button class="k-button" id="editNodeToSelected" style="margin:5px;width:79%;" >Update
								Tariff</button> -->
								<button type="submit"  id="editNodeToSelected" class="buttonS bLightBlue" value="Update Tariff">
						          Update	Tariff</button> 
...
/* $(document).ready(function() {
      ...
      $("#validFromu").kendoDatePicker();
      $("#validFromu").attr("disabled","disabled");
  });
  
$(document).ready(function() {
    
    $("#todateu").kendoDatePicker();
    $("#todateu").attr("disabled","disabled");
}); */
...
function startChangeu() {
    ...
    function endChangeu() {
		...
		var validFromu=$("#validFromu").val();
   	  ...
   	  if(new Date(validFromu)>new Date(endDate)){
			alert("valid to date cannot be less than valid from date");
			$("#todateu").data("kendoDatePicker").value("");
			return false;
	 } 	 
...
function getMessage(input) {
  return input.data("message");
	...
	$.ajax({
		type : "GET",
		url : "./tariffMaster/getDetails/"+nodeid,
		success : function(response)
		{
		...
		return $(kitem).find('>div span.k-in').text();
			...
			if (selectedNode.length == 0) {
		 	...
		 	function updatediv(){
		 		...
		 		var result=securityCheckForActionsForStatus("./Tariff/Electricity/TariffMaster/updateTariffMaster"); 
		 		var treeview = $("#treeview").data("kendoTreeView");
		 		if(result=="success"){ 
		 			var selectedNode = treeview.select();
					if(selectedNode.length == 0){
