...
<c:url value="/swTariff/read" var="transportSolidWasteTariffReadUrl" />
...
<div style="padding-left: 50px" class="fluid">
	...
	<div class="widget grid8" >
		<div class="whead">
			...
			<h6>Solid Waste Tariff Master</h6>
			<div class="titleOpt" >
		...
		<kendo:treeView name="treeview" dataTextField="solidWasteTariffName"
			select="onSelect" 
			template=" #: item.solidWasteTariffName #  <br>#:item.status # <input type='hidden' id='syed' class='#: item.solidWasteTariffName ##: item.solidWasteTariffId #' value='#: item.solidWasteTariffId#'/>">
			
								
			<kendo:dataSource>
				<kendo:dataSource-transport>
					<kendo:dataSource-transport-read url="${transportSolidWasteTariffReadUrl}"
				...
				<kendo:dataSource-schema>
					<kendo:dataSource-schema-hierarchical-model id="solidWasteTariffId"
				...
				<table style="height: 350px;">
					...
					<tr>
						...
						<td><kendo:dropDownList name="status" id="status" dataTextField="name"
								dataValueField="value" required="required"  optionLabel="Status">								
						...
						if (nodename != "") {
							...
							$
									.ajax({
										...
										url : "./swTariff/create",
										data : {
											solidWasteTariffId : nodeid,
											solidWasteTariffName : nodename,
											solidWastetreeHierarchy : treeHierarchy,
											status : status,
											solidWastetariffDescription : tariffdescription,
											solidWasteTariffCode        :  tariffcode,
											solidWastetariffNodetype: tariffnodetype,
											validFrom:       validFrom,
											validTo:  validTo 
											 },
  ...
  <div id="updateperm" title="Update Tariff" align="center" style="display: none"> 
		<br/>	
				...
				<table style="height: 350px;">
					...
					<tr>
					<td>From-Date*</td>
					<td> <kendo:datePicker format="yyyy/MM/dd " name="validFromu" id="validFromu" value="${today}" 	change="startChangeu" 	class="validate[required]" required="required"></kendo:datePicker><td>
					</td>
					</tr>
					...
					<td><kendo:dropDownList name="tariffnodetypeu" id="tariffnodetypeu" dataTextField="name"
								dataValueField="value"  optionLabel="Tariff Node Type"
								 required="required">
								<kendo:dataSource data="${tariffNodetype}" ></kendo:dataSource>
							</kendo:dropDownList></td>
					<tr>
...
$("#updateform").submit(function(e) {
 ...
 function updateform(){	
				...
				$
						.ajax({
							...
							data : {
								...
								solidWastetreeHierarchy : treeHierarchy,
								status : statusu,
								solidWastetariffDescription : tariffdescriptionu,
								solidWasteTariffCode        :  tariffcodeu,
								solidWastetariffNodetype: tariffnodetypeu,
	...
	$.ajax({
		type : "GET",
		url : "./swTariffMaster/getDetails/"+nodeid,
		success : function(response)
		{
		
			
			$('#editNodeText').val(response.solidWasteTariffName);
			$("#tariffcodeu").val(response.solidWasteTariffCode);
			$("#tariffdescriptionu").val(response.solidWastetariffDescription);
			
			 var dropdownlist1 = $("#tariffnodetypeu").data("kendoDropDownList");
				
				 dropdownlist1.search(response.solidWastetariffNodetype); 
	...
	texts = $.map(kitems, function(kitem) {
		return $(kitem).find('>div span.k-in').text();
			...
			//	alert("click");
		 	...
		 	function adddiv(){
		 		var result=securityCheckForActionsForStatus("./Tariff/SolidWaste/TariffMaster/createTariffMaster"); 
				if(result=="success"){ 
		 	...
		 	function updatediv(){
		 		...
		 		var result=securityCheckForActionsForStatus("./Tariff/SolidWaste/TariffMaster/updateTariffMaster"); 
		 		var treeview = $("#treeview").data("kendoTreeView");
		 		if(result=="success"){ 
		 			var selectedNode = treeview.select();
					if(selectedNode.length == 0){
						...
						$("#removeNode")
								.click(
										function() {
									 		...
									 		var result=securityCheckForActionsForStatus("./Tariff/SolidWaste/TariffMaster/deleteTariffMaster"); 
											if(result=="success"){ 
