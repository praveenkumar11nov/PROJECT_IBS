...
<c:url value="/users/rolesFilter" var="rolesFilter" />
<c:url value="/users/groupsFilter" var="groupsFilter" />
	...
	<kendo:grid name="grid" pageable="true" detailTemplate="ownerTemplate" change="onChange"  dataBound="userDataBound"
		...
		filterable="true" groupable="true" edit="parentedit">
		...
		<kendo:grid-toolbar>
		<kendo:grid-toolbarItem name="userTemplatesDetailsExport" text="Export To Excel" /> 
         <kendo:grid-toolbarItem name="userPdfTemplatesDetailsExport" text="Export To PDF" /> 
		...
		<kendo:grid-columns>
			...
			<kendo:grid-column title="Groups" field="groups"
				editor="groupsEditor" width="150px">
			...
			<kendo:grid-column title=""
				template="<a href='\\\#' id='temPID#=urId#' onClick='changeStatus(this.text)' class='k-button k-button-icontext btn-destroy k-grid-Activate#=data.urId#'>#= data.status == 'Inactive' ? 'Activate' : data.status == 'Inprogress' ? 'Approved' : data.status == 'Approved' ? 'De-activate' : data.status == 'Active' ? 'De-activate' : data.status == 'Rejected'? 'Activate' : 'Activate' #</a>"
				width="100px" />
							...
							<kendo:grid-pageable pageSize="10"></kendo:grid-pageable>
        						<kendo:grid-columns>
									    ...
									    <%--  <kendo:grid-column title="Person Name" editor="PersonNames" field="personId"></kendo:grid-column> --%>
									     <kendo:grid-column title="Access Type" field="acType" width="40px"></kendo:grid-column>
									     <kendo:grid-column title="Access Card Number"  field="acNo" width="40px"></kendo:grid-column>
									     ...
									     <kendo:grid-column title="Status"  field="status" width="40px"></kendo:grid-column>
        						</kendo:grid-columns>
        						 <kendo:dataSource >
						            <kendo:dataSource-transport>
                ...
                <kendo:tabStrip-item-content>
                    <div class='wethear'>
                    	...
                    	<br/>
								...
								resizable="true" sortable="true" reorderable="true"
								selectable="true" scrollable="true" edit="accessCardPermissionEvent">
								<kendo:grid-pageable pageSize="10"></kendo:grid-pageable>
        						...
        						<kendo:grid-columns>
        						  ...
        						  <kendo:dataSource requestEnd="AccessCardsPermissionRequestEnd">
						            <kendo:dataSource-transport>
...
<%-- 					                        <kendo:dataSource-schema-model-field name="createdBy" type="string" editable="false">
	...
	<script>
	  ...
	  $("#grid").on("click",".k-grid-userTemplatesDetailsExport", function(e) {
		  window.open("./userTemplate/userTemplatesDetailsExport");
	  });
	  
	  $("#grid").on("click",".k-grid-userPdfTemplatesDetailsExport", function(e) {
		  window.open("./userPdfTemplate/userPdfTemplatesDetailsExport");
	  });
	...
	function accessCardPermissionEvent(e)
	{
		  //alert(JSON.stringify(e.model));
		  	...
		  	$('label[for=acPointName]').parent().remove();  			
	
		 	$('input[name="acPointName"]').parent().remove();
	  ...
	  function statusEditor(container,options)
	 {
		...
		function edit(e) {	 
			...
			$(".k-edit-field").each(function () {
				$(this).find("#temPID"+urId).parent().remove();
		   	});
